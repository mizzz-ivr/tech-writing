...
もっと一般的な例なら、問い合わせの振り分けにも使えそう。

例えばサービスにこんな問い合わせが来る。

> 昨日決済したのに、有料プランが反映されていません。
...
単なるキーワード一致だと、「決済」という文字があるからbilling、くらいしか見られない。

でも実際の問い合わせはもっと曖昧。

> Proにしたはずなのに機能が使えない

みたいに「決済」と一言も書いていないこともある。

こういう**文章の意味を見て、決められた選択肢へ振り分ける**用途はかなりイメージしやすい。

公式ユースケースにもIntent Routing、Content Moderation、Invoice Classification、CSV Validation、Claim Verificationなどがある。

Jevの立ち位置は、やっぱり**何かを生成するより「どれ？」「どうする？」を決める側**っぽい。
