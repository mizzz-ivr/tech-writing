...
def load_policy(path: Path = CONFIG_PATH) -> FunnelPolicy:
    ...
    normalized_labels = frozenset(str(value).casefold().strip() for value in labels if str(value).strip())
...
def collect_snapshot(
    ...
    *,
    ...
    cutoff = datetime.combine(as_of - timedelta(days=policy.lookback_days), datetime.min.time(), tzinfo=JST).astimezone(timezone.utc)
    release_cutoff = datetime.combine(as_of - timedelta(days=policy.release_lookback_days), datetime.min.time(), tzinfo=JST).astimezone(timezone.utc)
    ...
    for repository in policy.repositories:
        ...
        pulls = fetcher(repo_api(repository, "/pulls?state=closed&sort=updated&direction=desc&per_page=30"))
        releases = fetcher(repo_api(repository, "/releases?per_page=15"))
        issues = fetcher(repo_api(repository, "/issues?state=closed&sort=updated&direction=desc&per_page=30"))
        if not isinstance(pulls, list) or not isinstance(releases, list) or not isinstance(issues, list):
            raise ValueError(f"unexpected GitHub collection response: {repository}")

        merged_pull_requests = []
        for item in pulls:
            merged_at = item.get("merged_at") if isinstance(item, dict) else None
        ...
        for item in issues:
            ...
            matched = sorted(item_labels.intersection(policy.significant_issue_labels))
            if not matched:
                continue
...
def validate_snapshot(payload: dict[str, Any], policy: FunnelPolicy) -> None:
    ...
    for row in repositories:
        ...
        if repository not in policy.repositories:
            raise ValueError(f"snapshot repository is outside allowlist: {repository}")
    ...
    for path in reversed(snapshot_paths()):
        try:
            ...
            if not isinstance(payload, dict):
                continue
            validate_snapshot(payload, policy)
            return payload
...
def normalize_title(value: str) -> str:
    ...
    value = re.sub(r"^(?:feat|fix|chore|docs|refactor|perf|test)(?:\([^)]*\))?\s*[:：]\s*", "", value)
...
def flatten_evidence(snapshot: dict[str, Any] | None) -> list[Evidence]:
    ...
    for repo in snapshot.get("repositories", []):
        ...
        for item in repo.get("merged_pull_requests", []):
            result.append(Evidence(repository, "pull_request", str(item.get("title") or ""), str(item.get("url") or ""), str(item.get("merged_at") or ""), f"#{item.get('number')}"))
        for item in repo.get("releases", []):
            title = str(item.get("name") or item.get("tag_name") or "")
            result.append(Evidence(repository, "release", title, str(item.get("url") or ""), str(item.get("published_at") or ""), str(item.get("tag_name") or "")))
        for item in repo.get("significant_issues", []):
            result.append(Evidence(repository, "issue", str(item.get("title") or ""), str(item.get("url") or ""), str(item.get("closed_at") or ""), f"#{item.get('number')}"))
    return result
...
def tracking_titles(articles: Iterable[Any], backlog: Iterable[Any]) -> list[tuple[str, str]]:
    result: list[tuple[str, str]] = []
    for article in articles:
        title = str(getattr(article, "title", "") or "").strip()
        if title:
            status = str(getattr(article, "effective_status", "article") or "article")
...
def build_funnel_payload(
    ...
    *,
    ...
    candidates.sort(key=lambda row: (KIND_PRIORITY.get(str(row["kind"]), 0), str(row["occurred_at"]), str(row["repository"]), str(row["title"])), reverse=True)
    untracked = [row for row in candidates if row["tracking_status"] == "untracked"]
...
def write_snapshot(policy: FunnelPolicy) -> Path:
    ...
    if target.exists():
        validate_snapshot(json.loads(target.read_text(encoding="utf-8")), policy)
        return target
...
def main() -> int:
    ...
    try:
        ...
        if args.refresh:
            ...
            print(f"GitHub writing funnel snapshot: {path.relative_to(analytics.ROOT)}")
        else:
            payload = build_funnel_payload([], [], policy=policy)
            print(f"GitHub writing funnel check: {len(policy.repositories)} repositories, {count} snapshots, {payload['evidence_count']} evidence rows")
