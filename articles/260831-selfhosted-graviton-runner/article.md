...
- hosted runnerに少し課金して復旧させる
- Repositoryをpublicにする（publicなら無制限無料）
- どこかにセルフホストrunnerを立てる

publicにするのはこのプロジェクトの都合で無し。課金は一番手軽ですが、毎月の変動費が増えるのは避けたい、という気持ちがありました。

決め手になったのは、**セルフホストrunnerの実行時間はGitHubの分数課金に含まれない**という点です。走らせるマシンのコストだけで済みます。しかも今回の `runner_id=0` 問題そのものも、hosted runnerを使わなくなれば消えます。

置き場所は、別で動かしているOracle Cloud（Minecraft用）も一瞬考えましたが、あの環境にCIを混ぜたくなかったのでやめました。結局、他のサービスをすでに載せているAWSに、CI専用のインスタンスを1台足すことにしました。
