...
CIを動かす場所を用意する方法は、思ったよりいくつもあります。今回検討したのは次のあたりです。

**1. GitHub Proにして課金する。** GitHub Proは月$4で、privateリポジトリのActions無料枠が2,000分から3,000分に増えます。それを超えた分はLinuxで1分$0.006（2026年1月の値下げ後のレート）。一番手軽で、CIの構成は何も変えなくていい。ただ、うちのCIは型チェック・テスト・Lint・複数のNext.jsビルド・OpenNextバンドル・Terraform validateまで回すので、1 run十数分。PRを普通に開いていくと3,000分はそれなりに現実的に見えてきて、そうなると使った分だけ請求が伸びる変動費になります。

**2. pay-as-you-goで従量課金だけ有効にする。** Proにせず、spending limitだけ上げて溢れた分を払う形。実質1と同じで、青天井の変動費という点が気になりました。

**3. リポジトリをpublicにする。** publicならhosted runnerは無制限で無料。これは効くんですが、このプロジェクトはpublicにできない都合があるので最初から外しました。

**4. 手元のマシンや常時起動のPCにrunnerを置く。** コストはゼロに近い。でも自分の開発機だと、CIが走っている間うるさいし重いし、電源を切ったらCIも止まる。専用の常時起動マシンを持っていないので、これも無し。

**5. Oracle CloudのAlways Free枠に置く。** ARMインスタンスが無料で使える枠があります。実際にMinecraft用のサーバをそこで動かしています。ただ、その環境にCIを相乗りさせたくなかった（デプロイ系のジョブも動くので、Minecraftサーバと同じホストに置くのは境界として気持ち悪い）。無料枠のARM在庫が取りにくい、という事情も知られています。

**6. AWSにCI専用のインスタンスを立てる。** 他のサービスをすでにAWSに載せているので、運用の土地勘がある。CI専用に1台足すだけなら、既存の請求に数百円乗るくらい。ここに落ち着きました。

ざっくり並べるとこんな感じです（月額はうちのCI量・東京リージョンでの概算で、正確な数字ではありません）。

| 方法 | 月額の目安 | 立てる手数 | 保守 | 備考 |
|---|---|---|---|---|
| GitHub Pro / 従量課金 | $4〜（超過分は従量） | ほぼゼロ | ゼロ | CIが増えると変動費が伸びる |
| public化 | 無料 | ゼロ | ゼロ | 今回は不可 |
| 手元 / 常時PC | ほぼ無料 | 小 | 中（電源・OS） | マシンを占有される |
| OCI Always Free | 無料 | 中 | 中 | 既存用途と相乗りしたくなかった／ARM在庫 |
...
> 補足（2026年に入ってからの動き）: GitHubは2026年1月にhosted runnerを最大39%値下げし、いっぽうで「セルフホストrunnerの実行時間も無料枠を消費させる（超過分は1分$0.002）」という変更を一度アナウンスしましたが、反発を受けて無期限延期になっています。この記事の「分数課金に含まれない」という前提は執筆時点のもので、公開前に最新の料金ページで見直します。
...
次のscale-to-zero化と、その移行で踏んだ穴（instance profile の名前衝突、2GB マシンでの OOM、未マージの infra ブランチを個別に apply して壊した話）は、Zenn の別記事にまとめました。
